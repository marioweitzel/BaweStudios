/**
 * OpenCodeHostAdapter - Adapter that delegates OpenCode CLI execution to a
 * local Windows bridge outside Docker. Hermano paralelo de
 * CodexHostAdapter/ClaudeHostAdapter: mismo contrato IHostAdapter, no
...
const SESSION_LOG = path.join(LOG_DIR, 'opencode-session.log');
const OPENCODE_CONFIG_PATH = path.join(__dirname, '..', '..', 'hosts', 'opencode.json');
...
function loadOpenCodeBridgeConfig(): OpenCodeBridgeConfig {
  const raw = fs.readFileSync(OPENCODE_CONFIG_PATH, 'utf8');
  const config = JSON.parse(raw) as OpenCodeBridgeConfig;
  const bridgeUrl = process.env.OPENCODE_BRIDGE_URL || config.bridgeUrl;
  const cwd = process.env.OPENCODE_CWD || config.cwd;
  const command = process.env.OPENCODE_COMMAND || config.command;

  if (config.type !== 'http-bridge') {
    throw new Error(`[OPENCODE HOST] opencode.json debe usar type="http-bridge"`);
  }
  if (!bridgeUrl) {
...
export class OpenCodeHostAdapter implements IHostAdapter {
  private config: OpenCodeBridgeConfig;
  ...
  async send(sessionId: string, message: string, options?: { markers?: { start: string; end: string } }): Promise<void> {
    ...
    slog(`[OPENCODE HOST] mensaje recibido desde chat: ${message}`);
    ...
    try {
      ...
      if (!this.activeSessions.has(sessionId) || this.stoppingSessions.has(sessionId)) {
        slog(`[OPENCODE HOST] respuesta ignorada para sesion detenida: ${sessionId}`);
        return;
      }
      ...
      if (options?.markers) {
        ...
        if (!extracted) {
          ...
          slog(`[OPENCODE HOST] ${errMsg}: ${responseText.slice(0, 200)}`);
          const errorMessage = { sessionId, type: 'error', text: GENERIC_HOST_ERROR_MESSAGE };
          this.errorCallbacks.forEach(cb => cb(sessionId, new Error(errMsg)));
          this.messageCallbacks.forEach(cb => cb(sessionId, errorMessage));
          return;
      ...
      const responseMessage: Record<string, unknown> = { sessionId, type: 'question', id: 'opencode-response', text: responseText };
      ...
      if (!this.activeSessions.has(sessionId) || this.stoppingSessions.has(sessionId)) {
        slog(`[OPENCODE HOST] error ignorado para sesion detenida: ${sessionId}: ${error.message.slice(0, 200)}`);
        return;
      }
  ...
  async stop(sessionId: string): Promise<void> {
    ...
    slog(`[OPENCODE HOST] stopping sessionId=${sessionId}`);
    ...
    try {
      ...
      slog(`[OPENCODE HOST] ${statusLabel} sessionId=${sessionId} stopStatus=${res.status} body=${bodyText.slice(0, 300)}`);
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err));
      slog(`[OPENCODE HOST] stop_error sessionId=${sessionId}: ${error.message.slice(0, 300)}`);
    } finally {
      this.activeSessions.delete(sessionId);
      this.stoppingSessions.delete(sessionId);
      console.log(`[OPENCODE HOST] sesion detenida: ${sessionId}`);
  ...
  private async callBridge(sessionId: string, prompt: string): Promise<{ text: string | null; pid: number | null }> {
    ...
    try {
      ...
      const bodyText = await res.text();
      slog(`[OPENCODE HOST] bridge status=${res.status} body=${bodyText.slice(0, 500)}`);
